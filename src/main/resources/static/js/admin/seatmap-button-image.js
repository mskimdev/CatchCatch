(() => {
    ...
    function bindEvents() {
        ...
        bindClick("downloadButtonImage", downloadButtonImage);
        ...
        bindClick("go3", () => runAutoConversion(false));
        bindClick("undoAction", undo);
        bindClick("redoAction", redo);
        bindClick("zoomIn", () => setZoom(state.zoom + 0.1));
        bindClick("zoomOut", () => setZoom(state.zoom - 0.1));
    ...
    async function saveButtonImageToServer(options = {}) {
        ...
        const htmlText = createPreviewHtml(imageDataUrl, meta);
        ...
        try {
            ...
            if (!options.silent) {
                toast(`파일 저장 완료: ${result.imageFile || result.imageUrl || "이미지"}`);
            }
            return true;
        } catch (error) {
            console.error(error);
...
<body>
        ...
        document.querySelectorAll(".button-image-step").forEach((section) => {
            ...
            if (status) {
                if (itemStep < nextStep) {
                    ...
                    status.textContent = nextStep === 2 ? "선택중" : (nextStep === 3 ? "보정중" : "진행중");
                } else {
    ...
    function onCanvasUp() {
        // 구역 선택 방식에서는 클릭 이벤트에서만 선택을 처리한다.
    }
