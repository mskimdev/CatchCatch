(function () {
  function initGenreFilter() {
    ...
    document.querySelectorAll('#genre-filter button').forEach(function (button) {
      const targetGenre = button.getAttribute('data-genre');
      button.classList.toggle('is-active', targetGenre === currentGenre);
      button.addEventListener('click', function () {
        if (targetGenre === 'all') {
          location.href = '/concerts/open-soon';
        } else {
          location.href = `/concerts/open-soon?genre=${targetGenre}`;
        }
      });
    });
  }
  ...
  function initPosterFallback() {
    document.querySelectorAll('.concert-poster').forEach(function (img) {
      img.addEventListener('error', function () {
        this.src = '/images/sample/default-poster.svg';
      }, { once: true });
    });
  }
