...
VALUES ('k6 부하테스트 전용 공연장', '서울특별시 송파구 올림픽로 000', 1000, NULL, NOW());

-- ================
--  concert_tb (k6 전용 콘서트)
-- ================
...
VALUES
    ((SELECT id FROM venue_tb WHERE name = 'k6 부하테스트 전용 공연장'),
     'k6 부하테스트 콘서트', 'k6', 'k6 부하 테스트 전용 콘서트 데이터.', '/images/sample/poster-music.svg', 'OPEN',
     '콘서트', 'concert', '2026-12-31', '2026-12-31', '2026-01-01 00:00:00', '전체 관람가', '120분', 'k6', '000-0000-0000',
     '/images/sample/detail-banner.svg', 'k6 부하테스트', 'k6 부하 테스트 전용', 'k6 부하 테스트 전용',
     100000, 80000, 60000, 40000, NOW(), false);

-- ================
--  concert_session_tb (k6 전용 회차 1개)
-- ================
INSERT INTO concert_session_tb
(concert_id, session_date, session_time, round, created_at, is_deleted)
VALUES
    ((SELECT id FROM concert_tb WHERE title = 'k6 부하테스트 콘서트'),
     '2026-12-31', '20:00:00', '1회차', NOW(), false);

-- ================
--  seat_tb (k6 전용 좌석 1000석)
--  capacity = min(FIXED_SYSTEM_LIMIT=500, AVAILABLE 좌석 수) 가 500에서 막히는 것까지 확인 가능하도록
